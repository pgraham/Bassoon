 ...
 * @package Bassoon_Test
 */
class ProxyTemplateTest extends \PHPUnit_Framework_TestCase {

  public function testProxyOutput() {
    $info = new RemoteService('BassoonTest\Mock\RemoteServiceImpl');
    $defPath = $info->getServiceDefinitionPath();

    $mthds = array();
    foreach ($info->getMethods() AS $mthd) {
      $mthds[$mthd->getName()] = new Template\ProxyMethod($mthd);
    }
    ...
    $proxyTemplate = new Template\Proxy($info);
    $output = $proxyTemplate->__toString();

    $expd['proxy'] = 'var BassoonTest_Mock_RemoteServiceImpl=function(){'
      . 'var p=\'' . dirname($defPath)
      . '/gen/ajx/BassoonTest_Mock_RemoteServiceImpl/\';'
      . 'return{' . implode(',', $expd) . '};}();';
  ...
  public function testPostProxyOutput() {
    $info = new RemoteService('BassoonTest\Mock\PostService');
    $mthds = $info->getMethods();

    $postParams = new Template\ProxyMethod($mthds[0]);
